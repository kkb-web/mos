...
import  {getVipSubjectList,  createVipCourse,  checkVipcourseName}  from  "../../../api/vipCourseApi";
...
class  Local  extends  React.Component  {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (this.state.vipTitleState == 'success' && this.state.subjectState == 'success' && this.state.vipDescriptionState == 'success' && this.state.vipPriceState == 'success' && this.state.courseThumbnailStatus == 'success' && this.state.friendCircleState == 'success' && this.state.groupSharingState == 'success' && this.state.shareDescribeState !== 'error' && this.state.thumbnailStatus == 'success') {
          params = {
            ...
            type: Number(values.courseType),   //  等待上线
            formName:values.tableName ? values.tableName : null,
            formUrl:values.tableName ? values.tableUrl : null
          };
          this.setState({
            disableBtn: true
          });
          createVipCourse(params).then(res => {
            this.setState({
              disableBtn: false
            });
            if (res.data.code == 0) {
  ...
  //课程名称失去焦点
  titleCheckRepeat = (e) => {
    checkVipcourseName(`name=${e.target.value}`).then(res => {
  ...
  validatorTitleCheck = (rule, val, callback) => {
    if (!(val.indexOf(" ") === -1)) {
      callback('禁止输入空格！');
    } else if (emojiRule.test(val)) {
      callback('禁止输入emoji！');
  ...
  validatorChooseSubject = (rule, val, callback) => {
    if (val) {
      this.setState({
        subjectValue: val
      })
  ...
  validatordescriptionCheck = (rule, val, callback) => {
    if (!(val.indexOf(" ") === -1)) {
      callback('禁止输入空格！');
    } else if (emojiRule.test(val)) {
      callback('禁止输入emoji！');
  ...
  validatorPriceCheck = (rule, val, callback) => {
    if (parseFloat(val) >= 0.01 && parseFloat(val) <= 100000.00) {
      callback();
      this.setState({
        vipPriceHint: '',
      ...
      callback('请输入有效范围内的数字 0.01~100000.00');
    }
  };
  ...
  validatorGroupSharingCheck = (rule, val, callback) => {
    if (val.length == 20) {
      callback('字数需在20字以内');
  ...
  validatorShareDescribeCheck = (rule, val, callback) => {
    if (val.length == 50) {
      callback('字数需在50字以内');
    } else if (val.length > 30) {
      callback('群分享描述建议少于30字');
    } else if (!(val.indexOf(" ") === -1)) {
      callback('禁止输入空格');
    } else {
      callback();
  ...
  validatorFriendCircleCheck = (rule, val, callback) => {
    if (val.length == 20) {
      callback('字数需在20字以内');
    } else if (val.length > 30) {
      callback('朋友圈分享标题建议少于30字');
    } else if (!(val.indexOf(" ") === -1)) {
      callback('禁止输入空格');
    } else {
      callback();
  ...
  validatorbeforeUploadCourseThumbnail = (rule, val, callback) => {
    if (val.type === 'image/jpg' || val.type === 'image/jpeg' || val.type === 'image/png') {
      callback();
    } else {
      callback('');
    }
  ...
  validatorThumbnail = (rule, val, callback) => {
    if (val.type === 'image/jpg' || val.type === 'image/jpeg' || val.type === 'image/png') {
      if (val.size > 10068) {
        callback('');
  ...
  render() {
    const {priceValue, subjectValue} = this.state;
    ...
    return (
      <div ref={node => this.node = node}>
        ...
        <div className="page-nav">
          ...
          <p className="title-style">新建VIP课</p>
          <p className="title-describe">VIP课创建成功后，还需要继续创建班级信息。</p>
        </div>
          ...
          <Form layout="horizontal" style={{paddingBottom: '20px'}}>
            <FormItem key='subjectVals' className="open-course-form" label="课程分类"
              ...
              {getFieldDecorator('courseType', {
                ...
                >
                  <Option key='0' value='0'>VIP课程</Option>
                  <Option key='1' value='1'>低价小课</Option>
                </Select>
              )}
            </FormItem>
            <FormItem className="open-course-form" label="VIP课名称" {...FormItemLayout} hasFeedback
                      validateStatus={this.state.vipTitleState}
                      help={this.state.vipTitleHint}>
            ...
            <FormItem className="open-course-form" label="VIP课程介绍"
              ...
              {getFieldDecorator('vipdescription', {
                ...
                <Input maxLength={30} placeholder="建议30字以内（用于官网、小程序等" onChange={this.descriptionCheck}/>
              )}
            </FormItem>
