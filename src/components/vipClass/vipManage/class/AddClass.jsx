...
import {addVipClass, getOldCourses,getNewCourses, getVipCourseDetail, checkClassName} from "../../../../api/vipCourseApi";
...
class Local extends React.Component {
    ...
    componentDidMount() {
        ...
        this.getVipDetail();
        //price禁止滚轮滑动
        document.getElementById('price').onmousewheel = function (event) {
            return false
        };
        //classPrice禁止滚轮滑动
        document.getElementById('classPrice').onmousewheel = function (event) {
            return false
        };
    ...
    getVipDetail = () => {
        let _this = this;
        getVipCourseDetail({courseId: this.state.courseId}).then(res => {
            if (res.data.code === 0) {
    ...
    handleSubmit = () => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            params = {
                ...
                newCourseName: _this.state.newCourseName.split(':')[1],
    ...
    // 检查班次名称是否重复
    checkClassName = (e) => {
        checkClassName(`name=${e.target.value}&courseId=${this.state.courseId}`).then(res => {
        ...
        return (
            <div>
                ...
                <div className="page-nav">
                    ...
                    <p className="title-style">新建班次</p>
                    <p className="title-describe">班次创建成功后，将产生默认渠道，销售可根据需要创建自己的推广渠道链接，该链接适用于以后的班级招生。</p>
                </div>
                    ...
                    <Form layout="horizontal" style={{paddingBottom: '20px'}}>
                        ...
                        <div style={{position: 'relative'}}>
                            <FormItem className="vip-class-form" label="班次名称" validateStatus={this.state.titleState}
                                ...
                                {getFieldDecorator('classTitle', {
                                    ...
                                    <Input placeholder="如‘001期、012期（内部标识，用户不可见）" onBlur={this.checkClassName}
                        ...
                        <FormItem className="vip-class-form" label="班次描述" validateStatus={this.state.detailState}
                            ...
                            {getFieldDecorator('classDetail', {
                                ...
                                <Input placeholder="描述老师、开班计划等信息" onChange={this.detailCheck}/>
                            )}
                        </FormItem>
                        <div style={{position: 'relative'}}>
                            <FormItem className="vip-class-form" label="原价" validateStatus={this.state.classPriceState}
                                ...
                                {getFieldDecorator('classPrice', {
                                    ...
                                    <Input type="number" addonBefore="￥" placeholder={getNum(this.state.price, 2)}
                                           onBlur={this.changeClassPrice}
