...
import { addImg } from '../../../api/preSaleData'
...
class Picture extends React.Component {
  ...
  collectImg = (value, index) => {
    ...
    if (value.collectId) {
      ...
      collectTemplate({ templateId: value.id ,business:'MATERIALS'}).then(res => {
        if (res.data.code === 0) {
  ...
  render() {
    ...
    return (
      <div style={{ marginTop: '-15px' }}>
        <div id="pic-input" style={{ overflow: 'hidden', borderBottom: '1px solid #e8e8e8' }}>
          <div style={{ float: 'left' }} className="modal-title-second">
            <p style={{ marginTop: '5px' }}>
              <span
                className="pic-title pic-title_active"
                style={{ marginLeft: '45px', cursor: 'pointer' }}
                onClick={this.changeTitleStyle.bind(this, 0)}
              >
                全部
              </span>
              <span
                ...
                onClick={this.changeTitleStyle.bind(this, 1)}
              >
                收藏
              </span>
            </p>
          ...
          <div style={{ float: 'right', display: 'inline-flex' }}>
            <div className="uploadTip">
              ...
              <h6>
                图片数量：最多5000张，还可上传{' '}
                <span style={{ color: '#1890ff' }}>{5000 - parseInt(limitImgCount)}</span> 张
              </h6>
            </div>
            <Upload {...props}>
                    ...
                    <div key={value.id} style={{ position: 'relative', cursor: 'pointer' }}>
                      <div className="modal-pic-font modal-pic-font_hide" onClick={() => hidePicModal(value, index)}>
                        <Icon
                          type="check-circle"
                          theme="filled"
                          style={{ color: '#189ff0', fontSize: '26px', margin: '10px 0 0 115px' }}
                        />
                      </div>
                      <div style={{ position: 'absolute', height: 'auto', left: '0px', top: '115px' }}>
                        ...
                        >
                          <span className="black-back">
                            <Icon type="eye" className="white-icon" onClick={this.showImg.bind(this, value)} />
                          </span>
                        </Tooltip>
                        <Tooltip
                          placement="top"
                          title={!value.collectId ? '收藏' : '取消收藏'}
                          getPopupContainer={() => document.querySelector('.img-hint')}
                        >
                          <span className="black-back">
                            <Icon
                              ...
                              onClick={this.collectImg.bind(this, value, index)}
                            />
                          </span>
                        </Tooltip>
                        {value.isDelete === 1 ? (
                          <Popconfirm
                            ...
                            onConfirm={() => this.deleteImg(value, index)}
                          >
                            <Tooltip
                              placement="top"
                              title="删除"
                            ...
                            >
                              <span className="black-back">
                                <Icon type="delete" className="white-icon" />
                              </span>
                            </Tooltip>
                          </Popconfirm>
                        ) : (
                          <span className="black-back">
                            <Icon
                              type="delete"
                              className="white-icon"
                              style={{ color: 'rgb(103, 98, 98, 0.7)', cursor: 'default' }}
                            />
                          </span>
                        )}
                      </div>
                      <p
                        style={{
                          background: `url(${value.url}) no-repeat`,
                          backgroundSize: 'cover',
                          width: '150px',
                          height: '150px',
                          marginBottom: '0'
                        }}
                        onClick={() => showPicModal(value, index)}
                      />
                      <Input
                        disabled={value.isDelete === 1 ? false : true}
                        style={{ width: '150px', display: 'block', textOverflow: 'ellipsis' }}
                        value={value.name}
                        onBlur={this.editName.bind(this, value.id)}
        ...
        <p className="reset-pic" style={{ display: 'none' }} onClick={this.changeTitleStyle.bind(this, 0)}>
          重置图片
        </p>
