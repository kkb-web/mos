...
import UEditor02 from './UeditorDemo02'
...
import { addMaterials, getSubject, getCourses } from '../../api/preSaleData'
...
class AddSaleData extends Component {
  constructor(props) {
    super(props)
    this.state = {
      loading: false,
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if(!err){
        if (
          ...
          this.state.subjectListStatus == 'success' &&
          this.state.friendCircleState == 'success' &&
          this.state.groupSharingState == 'success' &&
          this.state.shareDescribeState == 'success' &&
          this.state.thumbnailStatus == 'success'
        ) {
          data = {
            title: values.title,
            subjects: [...this.state.chooseSubject].map(item => item + ''),
            description: values.description,
            courseId: [...this.state.vipCourse].map(item => item + ''),
            ...
            materialsShare: {
              ...
              shareCoverImage: this.state.shareThumbnails
            }
          };
          !err && this.setState({
            disableBtn: true
          });
          !err && addMaterials(data)
              .then(res => {
                this.setState({
                  disableBtn: false
                })
  ...
  shareDescribeCheck = e => {
    if (e.target.value.length === 0) {
      ...
      console.log("12")
      this.setState({
        shareDescribeHint: '群分享描述不得超出50字',
        shareDescribeState: 'error'
      })
    } else if (e.target.value.length >= 30) {
      this.setState({
    ...
    return (
      <div className="add-my-user">
        <div className="page-nav">
          ...
          <p className="title-style">新建资料</p>
          <p className="title-describe">
            请根据实际使用情况创建即可，创建完成后，每个资料销售可以创建自己的专属的资料链接，用户端会以文章详情页的方式打开。
          </p>
        </div>
        <Form>
          <Card title="基本信息" bordered={null}>
            ...
            <FormItem className="open-course-form" label="描述" validateStatus={this.state.preDataDescriptionState}
                      help={this.state.preDataDescriptionHint} {...FormItemLayout} hasFeedback>
              {getFieldDecorator('description', {
                rules: [{ required: false ,message: '最多允许输入50个字'}]
              })(
                <TextArea
                  rows={3}
          ...
          <Card title="资料详细信息" bordered={null} style={{ marginTop: '24px' }}>
            ...
            <UEditor ref="ueditor" type="init" initData={this.state.initData} />
            <input
        ...
        <div className="upload-title bottom-btn" style={{ zIndex: 1201 }}>
          ...
          <Button disabled={disableBtn} type="primary" onClick={this.onSubmit}>
            提交
          </Button>
        </div>
      </div>
...
const SaleData = Form.create()(AddSaleData)
