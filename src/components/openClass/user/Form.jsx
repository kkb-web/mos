...
export default class UForm extends Component {
    ...
    componentDidMount() {
        ...
        urlStuCondition(window.location.pathname.slice(12)).then(res => {
            this.setState({
                data: res.data.data
            });
                                ...
                                historyNewsDataMessage && historyNewsDataMessage.map((value, index) => {
                                    return (
                                        <div className="historynes">
                                            <p className="historynes-title">{value.createBy}通过《{value.title}》{value.business == 'MATERIALS' ? '资料包' : '公开课'}发送消息：</p>
                                            {
                                                value.msgType == 'text'
