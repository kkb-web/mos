...
import {addRole, checkRole, queryRole, getSubjectList} from "../../../api/roleApi";
...
class Local extends React.Component {
    ...
    componentWillMount () {
        ...
        subjectsOrigin = [];
        subjectList = [];
        queryRole(window.location.pathname.slice(26)).then(res => {
    ...
    createColumn = () => {
        ...
        columns = [{
            ...
            title: <span><Checkbox onChange={this.onCheckAllChange.bind(this, 'all')}
                                   className='subjects-all subjectAll allAll'
    ...
    changeSubValue = (name, i, subjectId, e) => {
        ...
        if (e.target.checked) {
            ...
            let allSub = document.querySelectorAll('.subjects-all .ant-checkbox');
            for (let j = 0; j < allSub.length; j++) {
                allSub[j].checked = false;
                allSub[j].classList.remove('ant-checkbox-checked');
            }
            checks[i].checked = false;
            spans[i].classList.remove('ant-checkbox-checked');
            for (let i = 0; i < this.state.data.length; i++) {
    ...
    handleSubmit = () => {
        ...
        validateFieldsAndScroll((errors, values) => {
            ...
            if (permissionIdList.length > 0) {
                ...
                message.error('角色权限提示：您设置的选项中还有未勾选的权限，将无法进行对应的操作！')
            }
        })
    };
    ...
    handleCancel = () => {
        ...
        validateFields((errors, values) => {
            console.log(values, values.roleName, values.roleDes, "===表单输入数据");
            if (values.roleName !== undefined || values.roleDes !== undefined || permissionIdList.toString() !== permissionIdOrigin.toString() || selfList.toString() !== selfOrigin.toString() || subjects.toString() !== subjectsOrigin.toString()) {
                this.setState({
                    visible: true
                })
            } else {
