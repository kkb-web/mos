...
import {Button, Card, Modal, Row, Col, Input, DatePicker, LocaleProvider, Select, Form, message, Table, Badge} from 'antd';
import BreadcrumbCustom from "../../common/BreadcrumbCustom";
import history from "../../common/History";
import moment from 'moment';
import zh_CN from "antd/lib/locale-provider/zh_CN";
import {disabledDate, formatDateTime, getToken, priceType,renderOrderState} from "../../../utils/filter";
import {
    checkUser,
    editOrderUser,
    getCourses,
    getOrderUserDetail,
    getSaleList, queryUserOrder
...
class EditOrderUser extends Component {
    constructor(props) {
        super(props);
        this.state = {
            userId: parseInt(window.location.pathname.slice(26)),
            outOrderId: this.props.location.outOrderId,
            saleList: [],
            courseList: [],
            courseStatus: '',
            nickStatus: '',
    ...
    getOrderUserDetail = () => {
        getOrderUserDetail({
            ...
            outOrderId: this.state.outOrderId
        }).then(res => {
    ...
    // 获取用户订单信息
    getUserOrder = () => {
      queryUserOrder(this.state.userId).then(res => {
          if (res.data.code === 0) {
              this.setState({
        ...
        if(this.state.mobileState){
            this.props.form.validateFieldsAndScroll((err, value) => {
                ...
                if (!err) {
                    ...
                    editOrderUser(params).then(res => {
                        if (res.data.code === 0) {
                            that.setState({
                                disableBtn: false
                            });
                            message.success('编辑成功');
                            history.push('/app/userCenter/orderuser')
                        } else {
                            message.error(res.data.msg)
                        }
                    })
    ...
    render() {
        const {saleList, courseList, courseStatus, nameStatus, nameHint, nickStatus, nickHint, disableBtn, visible, details, dataSource} = this.state;
        ...
        return (
            <div className="edit-order-user">
                ...
                <Form>
                        ...
                        <Row type="flex" justify="space-between">
                            {details && details.map((value, index) =>
                                <Col span={7} key={index}>
                                    <p style={{color: '#189ff0', minHeight: '40px', marginBottom: 0}}>{value.questionKey}</p>
                                    <p style={{minHeight: '40px'}}>{value.questionValue}</p>
                                </Col>
                            )}
                        </Row>
                    </Card>
                    ...
                    <Card title="订单信息" bordered={null} style={{marginTop: '24px'}}>
                        <Table
                            key={(record, i) => i}
                            rowKey={(record, i) => i}
                            columns={columns}
...
const myUser = Form.create()(EditOrderUser);
